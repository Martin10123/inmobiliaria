...
import { Save, GitBranchPlus, GitCommit, Edit2, Check, X, Plus, Trash2, UserPlus } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { Modal } from '@/components/ui/modal'
import type { RequestTypeConfig, ApprovalMode, ApprovalRule } from '@/types'
import { mockRequestTypeConfigs } from '../../requests/config/requestTypeConfigs'
import { mockApprovers } from '../../requests/data/mockApprovers'

export default function ApprovalFlowsTab() {
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs text-gray-600 flex items-center gap-2">
            <GitCommit className="w-3 h-3 inline" />Dependiente: secuencia | 
...
// Modal para Seleccionar Aprobadores del Tipo
function EditApproversModal({ 
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title={`Aprobadores - ${config.name}`} maxWidth="2xl">
      <div className="p-6 space-y-4">
              ...
              >
                <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${
                  ...
                  {isSelected ? (
                    ...
                    <span className="text-sm font-bold text-gray-600">{approver.name.charAt(0)}</span>
                  )}
                </div>
                <div className="flex-1 min-w-0">
...
function EditRuleModal({ 
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title="Editar Regla de Aprobación" maxWidth="2xl">
      <div className="p-6 space-y-6">
        <div>
          <h3 className="text-sm font-semibold text-gray-900 mb-3">Rango de Monto</h3>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-2">Monto Mínimo (opcional)</label>
            ...
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-2">Monto Máximo (opcional)</label>
              <div className="flex items-center gap-2">
                <span className="text-sm text-gray-600">$</span>
                <input
                  type="number"
        ...
        <div className="flex gap-3 pt-4 border-t border-gray-200">
          <Button onClick={() => onSave(editedRule)} className="flex-1">
            <Check className="w-4 h-4 mr-2" />
            Guardar Regla
          </Button>
          <button
